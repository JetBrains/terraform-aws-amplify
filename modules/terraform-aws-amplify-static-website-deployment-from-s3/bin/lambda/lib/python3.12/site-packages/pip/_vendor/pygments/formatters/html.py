...
class HtmlFormatter(Formatter):
    ...
    With the `standard` option, a complete HTML 4 document is output, including
    ...
    `standard`
        Tells the formatter to output a "standard" document, i.e. a complete
        self-contained document (default: ``False``).

    `title`
        If `standard` is true, the title that should be used to caption the
        document (default: ``''``).
    ...
    `cssfile`
        If the `standard` option is true and this option is given, it must be the
    ...
    `noclobber_cssfile`
        ...
        not be overwritten. This allows the use of the `standard` option in
    ...
    **Subclassing the HTML formatter**
    ...
    and/or "standard document" wrappers if the respective options are set. Then, all
